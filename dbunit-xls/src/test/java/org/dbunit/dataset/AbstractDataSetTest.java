...
public abstract class AbstractDataSetTest extends AbstractTest {
     ...
     * excludes tables with Identity columns (MSSQL) because they are specific to
     * MSSQL. TODO : should be refactored into the various DatabaseEnvironments!
     */
    public static IDataSet removeExtraTestTables(IDataSet dataSet) throws Exception {
        ...
        /*
         * this table shows up on MSSQLServer. It is a user table for storing diagram
         * information that really should be considered a system table.
         */
        ...
        final List<String> removeList = nameList.stream().filter(t -> t.startsWith("spt")).collect(Collectors.toList());
        nameList.removeAll(removeList);
        /*
         * These tables are created specifically for testing identity columns on MSSQL
         * server. They should be ignored on other platforms.
         */
    ...
    public void testGetTables() throws Exception {
        ...
        for (int i = 0; i < expected.length; i++) {
            assertEqualsTableName("name " + i, expected[i], tables[i].getTableMetaData().getTableName());
        }
    }
    ...
    public void testIterator() throws Exception {
        ...
        while (iterator.next()) {
            assertEqualsTableName("name " + i, expected[i], iterator.getTableMetaData().getTableName());
            i++;
        }
