...
/**
 * Abstract data set loader, which provides a basis for concrete implementations
 * of the {@link DataSetLoader} strategy. Provides a <em>Template Method</em>
 * based approach for {@link #loadDataSet(Class, String) loading} data using a
...
public abstract class AbstractDataSetLoader implements DataSetLoader {
    ...
    /**
     * Loads a {@link IDataSet dataset} from {@link Resource}s obtained from the
     * specified <tt>location</tt>. Each <tt>location</tt> can be mapped to a number
     * of potential {@link #getResourceLocations resources}, the first resource that
     * {@link Resource#exists() exists} will be used. {@link Resource}s are loaded
     * using the {@link ResourceLoader} returned from {@link #getResourceLoader}.
     * <p>
     * If no resource can be found then <tt>null</tt> will be returned.
     *
     * @see #createDataSet(Resource)
     * @see com.github.springtestdbunit.dataset.DataSetLoader#loadDataSet(Class,
    ...
    /**
     * Gets the {@link ResourceLoader} that will be used to load the dataset
     * {@link Resource}s.
     * 
    ...
    /**
     * Get the resource locations that should be considered when attempting to load
     * a dataset from the specified location.
     * 
     * @param testClass The class under test
