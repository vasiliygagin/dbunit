...
public class DbUnitTestExecutionListener extends AbstractTestExecutionListener {
    ...
    // TODO: probably want to save something in spring context for performance reasons
    ...
    private DatabaseConnections prepareDatabaseConnections(ApplicationContext applicationContext, DbUnitConfiguration configuration) {
    ...
    private Map<String, IDatabaseConnection> getDatabaseConnectionUsingCommonBeanNames(Map<String, IDatabaseConnection> allDatabaseConnections) {
    ...
    private Map<String, IDatabaseConnection> filterByNames(Map<String, IDatabaseConnection> connections,
        ...
        for (String name : names) {
            ...
            if (connection == null) {
                throw new IllegalArgumentException("IDatabaseConenction can not be found. Expecting Spring bean of type "
                        + IDatabaseConnection.class.getName() + " or " + DataSource.class.getName() + " with name \"" + name + "\"");
            }
    ...
    private Map<String, IDatabaseConnection> discoverDatabaseConnections(ApplicationContext applicationContext) {
        Map<String, IDatabaseConnection> databaseConnections = new HashMap<>(applicationContext.getBeansOfType(IDatabaseConnection.class));
    ...
    private DataSetLoader prepareDataSetLoader(ApplicationContext applicationContext, String dataSetLoaderBeanName,
        ...
        if (!StringUtils.hasLength(dataSetLoaderBeanName) && applicationContext.containsBean(DATA_SET_LOADER_BEAN_NAME)) {
    ...
    private DatabaseOperationLookup prepareDatabaseOperationLookup(Class<? extends DatabaseOperationLookup> databaseOperationLookupClass) {
    ...
    public void afterTestMethod(TestContext testContext) throws Exception {
        ...
        testException = runner.afterTestMethod(testClass, testInstance, testMethod, testException, databaseConnections, dataSetLoader, databaseOperationLookup);
