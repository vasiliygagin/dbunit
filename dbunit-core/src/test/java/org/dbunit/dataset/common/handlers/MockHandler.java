...
public class MockHandler extends AbstractPipelineComponent {
    private ExpectationCounter mySetSuccessorCalls = new ExpectationCounter(
	    "org.dbunit.dataset.csv.handlers.AbstractPipelineComponent SetSuccessorCalls");
    private ReturnValues myActualSetSuccessorReturnValues = new VoidReturnValues(false);
    private ExpectationList mySetSuccessorParameter0Values = new ExpectationList(
	    "org.dbunit.dataset.csv.handlers.AbstractPipelineComponent org.dbunit.dataset.csv.handlers.Handler");
    private ExpectationCounter myGetSinkCalls = new ExpectationCounter(
	    "org.dbunit.dataset.csv.handlers.AbstractPipelineComponent GetSinkCalls");
    private ReturnValues myActualGetSinkReturnValues = new ReturnValues(false);
    private ExpectationCounter myHandleCalls = new ExpectationCounter(
	    "org.dbunit.dataset.csv.handlers.AbstractPipelineComponent HandleCalls");
    private ReturnValues myActualHandleReturnValues = new VoidReturnValues(false);
    private ExpectationList myHandleParameter0Values = new ExpectationList(
	    "org.dbunit.dataset.csv.handlers.AbstractPipelineComponent char");
	...
	if (nextReturnValue instanceof ExceptionalReturnValue
		&& ((ExceptionalReturnValue) nextReturnValue).getException() instanceof IllegalInputCharacterException)
