...
public class StringDataTypeTest extends AbstractDataTypeTest {
    private final static DataType[] TYPES = { DataType.CHAR, DataType.VARCHAR, DataType.LONGVARCHAR,
//        DataType.CLOB,
    };
...
//            "CLOB",
    ...
    public void testTypeCast() throws Exception {
        Object[] values = { null, "bla", new java.sql.Date(1234), new java.sql.Time(1234), new java.sql.Timestamp(1234),
                Boolean.TRUE, new Integer(1234), new Long(1234), new Double(12.34),
                new byte[] { 'a', 'b', 'c', 'd' }, };
        String[] expected = { null, "bla", new java.sql.Date(1234).toString(), new java.sql.Time(1234).toString(),
                new java.sql.Timestamp(1234).toString(), "true", "1234", "1234", "12.34", "YWJjZA==", };

        assertEquals("actual vs expected count", values.length, expected.length);

        for (int i = 0; i < TYPES.length; i++) {
            for (int j = 0; j < values.length; j++) {
                assertEquals("typecast " + j, expected[j], TYPES[i].typeCast(values[j]));
            }
        }
    }
    ...
    private Object getBadClob() {
        ...
        // in jdk 1.4+
        java.lang.reflect.InvocationHandler alwaysThrowSqlExceptionHandler = new java.lang.reflect.InvocationHandler() {
        ...
        return java.lang.reflect.Proxy.newProxyInstance(java.sql.Clob.class.getClassLoader(),
                new Class[] { java.sql.Clob.class }, alwaysThrowSqlExceptionHandler);
    }
    ...
    public void testCompareEquals() throws Exception {
        Object[] values1 = { null, "bla", new java.sql.Date(1234), new java.sql.Time(1234),
                new java.sql.Timestamp(1234), Boolean.TRUE, new Integer(1234), new Long(1234), new Double(12.34),
                new byte[] { 'a', 'b', 'c', 'd' }, };
        String[] values2 = { null, "bla", new java.sql.Date(1234).toString(), new java.sql.Time(1234).toString(),
                new java.sql.Timestamp(1234).toString(), "true", "1234", "1234", "12.34", "YWJjZA==", };
    ...
    public void testCompareDifferent() throws Exception {
        Object[] less = { null, "", "abcd", "123", };

        Object[] greater = { "bla", "bla", "efgh", "1234", };
