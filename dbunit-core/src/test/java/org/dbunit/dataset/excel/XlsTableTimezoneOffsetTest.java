...
public class XlsTableTimezoneOffsetTest {
    private static final String EXCEL_SPREADSHEET = "/excel/XlsTableTimezoneOffset.xlsx";
    private static final String TABLE_NAME = "xxx_tz_offset_test";
    private static final String FAIL_MSG = "row '%d' column '%s' doesn't match.";
    ...
    /**
     * Simple test that loads the specified excel file and makes sure that the date
     * is correct if the JVM's time zone is different from GMT/UTC This test only
     * makes sense when the default time zone is NOT UTC (i.e. has an offset
     * different from 0), otherwise the test would succeed even if the bug is still
     * present. Thus, the precondition check makes sure that the default time zone
     * offset is not 0 and aborts the test if it is.
     *
     * To change the time zone offset to something other than 0, either change your
     * machine's system time zone setting, or use JVM parameter
