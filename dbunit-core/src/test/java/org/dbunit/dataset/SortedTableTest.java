...
public class SortedTableTest extends AbstractTableTest {
	...
	for (int i = 0; i < sortedTable.getRowCount(); i++) {
	    assertEquals("value row " + i, expected[i], sortedTable.getValue(i, columnName));
	}
    }
	...
	for (int i = 0; i < table.getRowCount(); i++) {
	    assertEquals("value row " + i, expected[i], table.getValue(i, columnName));
	}
    }
	...
	// The column actually used for sorting must has some different attributes than
	// the one passed in (dataType, nullable)
