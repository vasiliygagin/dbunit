...
public class DataSetUtilsTest extends TestCase {
	...
	assertEquals("prefix + name", "prefix.name", DataSetUtils.getQualifiedName("prefix", "name"));

	assertEquals("null prefix", "name", DataSetUtils.getQualifiedName(null, "name"));

	assertEquals("empty prefix", "name", DataSetUtils.getQualifiedName("", "name"));

	assertEquals("existing prefix", "prefix.name", DataSetUtils.getQualifiedName("wrongPrefix", "prefix.name"));

	assertEquals("escaped prefix + name", "prefix.name", DataSetUtils.getQualifiedName("prefix", "name"));
...
//        assertEquals(null, DataSetUtils.getEscapedName(null, "[?]"));
	...
	for (int i = 0; i < columns.length; i++) {
	    assertEquals("find column same", columns[i], DataSetUtils.getColumn("c" + i, columns));
	}
    }
	...
	ValueStringData[] values = new ValueStringData[] { new ValueStringData(null, DataType.REAL, "NULL"),
		...
		new ValueStringData(new Time(0L), DataType.TIME, "{t '" + new Time(0L).toString() + "'}"),
	...
	for (int i = 0; i < values.length; i++) {
	    ...
	    String valueString = DataSetUtils.getSqlValueString(data.getValue(), data.getDataType());
