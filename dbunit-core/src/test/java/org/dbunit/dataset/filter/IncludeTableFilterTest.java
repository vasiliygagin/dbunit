...
public class IncludeTableFilterTest extends AbstractTableFilterTest {
    ...
    static final String[] MATCHING_PATTERNS = { "?bcde", "?bc*", "*", "a?cde", "abcd?", "*e", "a*", "a*e", "a*d*e",
            "a**e", "abcde*", "*abcde", "?????", };
    static final String[] NONMATCHING_PATTERNS = { "?abcde", "abcde?", "*f*", "??????", "????", };

    public IncludeTableFilterTest() throws Exception {
    }
    ...
    public void testGetTableNamesWithPatterns() throws Exception {
        ...
        IDataSet dataSet = new DefaultDataSet(new ITable[] { new DefaultTable(MATCHING_NAME), new DefaultTable("toto"),
        ...
        for (String pattern : patterns) {
            ...
            if (pattern.equals("*")) {
                String[] actualNames = filter.getTableNames(dataSet);
                assertEquals("name count - " + pattern, dataSet.getTableNames().length, actualNames.length);
                assertEquals("names - " + pattern, Arrays.asList(dataSet.getTableNames()), Arrays.asList(actualNames));
            } else {
                String[] actualNames = filter.getTableNames(dataSet);
    ...
    public void testGetTablesWithPatterns() throws Exception {
        ...
        for (String pattern : patterns) {
            ...
            if (pattern.equals("*")) {
                ITable[] actualTables = DataSetUtils.getTables(filter.iterator(dataSet, false));
                String[] actualNames = new DefaultDataSet(actualTables).getTableNames();
                assertEquals("table count - " + pattern, dataSet.getTableNames().length, actualNames.length);
                assertEquals("table names - " + pattern, Arrays.asList(dataSet.getTableNames()),
