...
public class RelativeDateTimeParserTest {
    ...
    private static RelativeDateTimeParser parser = new RelativeDateTimeParser(CLOCK);
    ...
    public void testNullInput() throws Exception {
        try {
            ...
            assertTrue(e.getMessage().contains("Relative datetime input must not be null or empty."));
        }
    }
    ...
    private void verifyPatternMismatchError(String input) {
        try {
            ...
            assertEquals("'" + input + "' does not match the expected pattern [now{diff}{time}]. "
                    + "Please see the data types documentation for the details. "
                    + "http://dbunit.sourceforge.net/datatypes.html#relativedatetime", e.getMessage());
        }
    }
    ...
    public void testInvalidFormat_UnparsableTime() throws Exception {
        try {
            ...
            assertEquals("Text '1:23' could not be parsed at index 0", e.getMessage());
        }
    }
    ...
    public void testUnitResolution() throws Exception {
        ...
        LocalDateTime expected = LocalDateTime.now(CLOCK).plus(1, ChronoUnit.YEARS).plus(-2, ChronoUnit.MONTHS)
                .plus(3, ChronoUnit.DAYS).plus(-4, ChronoUnit.HOURS).plus(5, ChronoUnit.MINUTES)
    ...
    public void testOrderInsensitivity() throws Exception {
        ...
        LocalDateTime expected = LocalDateTime.now(CLOCK).plus(1, ChronoUnit.SECONDS).plus(-2, ChronoUnit.MINUTES)
                .plus(3, ChronoUnit.HOURS).plus(-4, ChronoUnit.DAYS).plus(5, ChronoUnit.MONTHS)
                .plus(-6, ChronoUnit.YEARS);
        assertEquals(actual, expected);
    }
    ...
    public void testWhitespaces() throws Exception {
        ...
        LocalDateTime expected = LocalDateTime.now(CLOCK).plus(1, ChronoUnit.YEARS).plus(-2, ChronoUnit.MONTHS).plus(3,
                ChronoUnit.DAYS);
        assertEquals(actual, expected);
    }
    ...
    public void testHoursMinutes() throws Exception {
        ...
        LocalDateTime expected = LocalDateTime.of(LocalDate.now(CLOCK), LocalTime.of(12, 34));
        assertEquals(actual, expected);
    }
    ...
    public void testHoursMinutesSeconds() throws Exception {
        ...
        LocalDateTime expected = LocalDateTime.of(LocalDate.now(CLOCK), LocalTime.of(2, 34, 56));
        assertEquals(actual, expected);
    }
}
