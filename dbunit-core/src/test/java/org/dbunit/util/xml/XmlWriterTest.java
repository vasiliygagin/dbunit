...
public class XmlWriterTest extends TestCase {
	...
	// Input should be equal to output because the text already starts with a CDATA
	// section
	assertEquals("<COLUMN1>" + text + "</COLUMN1>\n", actualXml);
    }

    public void testWriteNestedCDATAWithSurrounder() throws Exception {
	String text = "<myXmlText>" + XmlWriter.CDATA_START + "Text that itself is in a CDATA section"
		+ XmlWriter.CDATA_END + "</myXmlText>";
	String expectedResultText = "<myXmlText>" + XmlWriter.CDATA_START + "Text that itself is in a CDATA section]]"
	...
	String expectedXml = "<COLUMN1>" + XmlWriter.CDATA_START + expectedResultText + XmlWriter.CDATA_END
		+ "</COLUMN1>\n";
	assertEquals(expectedXml, actualXml);
    }
