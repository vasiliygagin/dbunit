...
//        testFlatXmlWriter();
         ...
         * XmlDataSet(TestUtils.getFileReader("xml/dataSetTest.xml")); InputSource
         * source = new
         * InputSource(TestUtils.getFile("xml/xmlTableTest.xml").toURL().toString()); //
         * InputSource source = new InputSource(new
         * File("writerTest.xml").toURL().toString()); FlatXmlProducer flatXmlProducer =
         * new FlatXmlProducer(source); XmlProducer xmlProducer = new
         * XmlProducer(source); MockDataSetProducer producer = new
         * MockDataSetProducer(); producer.setupColumnCount(4);
         * producer.setupRowCount(2); producer.setupTableCount(3); IDataSet dataSet =
         * new StreamingDataSet(xmlProducer); // IDataSet dataSet = new
         * StreamingDataSet(xmlProducer);
         * 
         * // System.out.println(connection.createDataSet());
         * 
         * // DatabaseOperation.INSERT.execute(connection, dataSet);
         * 
         * // IDataSet dataSet = connection.createDataSet(); // OutputStream out = new
         * FileOutputStream("c://writerTest.xml"); OutputStream out = System.out; //
         * FlatXmlWriter writer = new FlatXmlWriter(new OutputStreamWriter(out,
         * "UTF8")); XmlDataSetWriter writer = new XmlDataSetWriter(new
         * OutputStreamWriter(out, "UTF8")); writer.write(dataSet);
         */
...
//        System.setProperty("dbunit.name.escapePattern", "\"?\"");
        IDatabaseConnection connection = DatabaseEnvironmentLoader.getInstance(null).getConnection();
...
//                new FileOutputStream("src/dtd/flatXmlDataSetTest.dtd"));
     ...
     * 
     * public static void readWriteWorkbook() throws Exception { POIFSFileSystem fs
     * = new POIFSFileSystem(new FileInputStream("workbook.xls")); HSSFWorkbook wb =
     * new HSSFWorkbook(fs); HSSFSheet sheet = wb.getSheetAt(0); HSSFRow row =
     * sheet.getRow(2); HSSFCell cell = row.getCell((short)3); if (cell == null)
     * cell = row.createCell((short)3); cell.setCellType(HSSFCell.CELL_TYPE_STRING);
     ...
     * HSSFWorkbook(); HSSFSheet sheet = wb.createSheet("new sheet"); HSSFRow row =
     * sheet.createRow((short)2); row.createCell((short) 0).setCellValue(1.1);
     * row.createCell((short) 1).setCellValue(new Date()); row.createCell((short)
     * 2).setCellValue("a string"); row.createCell((short) 3).setCellValue(true);
     ...
     * sheet.createRow((short)0); // Create a cell and put a value in it. HSSFCell
     * cell = row.createCell((short)0); cell.setCellValue(1);
     * 
     ...
     * FileOutputStream("workbook.xls"); wb.write(fileOut); fileOut.close(); }
     * 
     * public static void createDateCells() throws Exception { HSSFWorkbook wb = new
     * HSSFWorkbook(); HSSFSheet sheet = wb.createSheet("new sheet");
     * 
     * // Create a row and put some cells in it. Rows are 0 based. HSSFRow row =
     ...
     * a date. HSSFCell cell = row.createCell((short)0); cell.setCellValue(new
     * Date());
     * 
     * // we style the second cell as a date (and time). It is important to create a
     * new cell style from the workbook // otherwise you can end up modifying the
     * built in style and effecting not only this cell but other cells.
     * HSSFCellStyle cellStyle = wb.createCellStyle();
     * cellStyle.setDataFormat(HSSFDataFormat.getBuiltinFormat("m/d/yy h:mm")); cell
     * = row.createCell((short)1); cell.setCellValue(new Date());
