...
public class DefaultVerifyTableDefinitionVerifier implements VerifyTableDefinitionVerifier {
    ...
    public void verify(final String tableName, final String[] columnExclusionFilters,
    ...
    protected void doVerify(final String tableName, final String[] columnExclusionFilters,
	    final Map<String, ValueComparer> columnValueComparers) {
    ...
    protected void failIfColumnValueComparersHaveExcludedColumn(final String tableName, final String columnName,
	    final Map<String, ValueComparer> columnValueComparers) {
	final ValueComparer valueComparer = columnValueComparers.get(columnName);
	if (valueComparer == null) {
	    log.trace("failIfColumnValueComparersHaveExcludedColumn:" + "config ok as no valueComparer found"
		    + " for excluded columnName={}", columnName);
	} else {
	    final String msg = "Test setup conflict: table=" + tableName + ", columnName=" + columnName
		    + ", has a VerifyTableDefinition column exclusion" + " and a specific column ValueComparer="
		    + valueComparer + "; to test the column, remove the exclusion;"
