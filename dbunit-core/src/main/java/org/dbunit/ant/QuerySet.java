...
/**
 * This element is a container for Queries. It facilitates reuse through
 * references. Using Ant 1.6 and greater, references can be defined in a single
 * build file and <i>import</i>ed into many others. An example of where this is
 * useful follows:
 * <p>
 * In our database we have INDIVIDUALS which must have an associated NAME_INFO
 * and at least one IND_ADDRESS. The developer creating a dataset for his/her
 * tests probably won't know all the details of what relationships are expected,
 * and if he did, its an error prone and repetitive task to create the correct
 * SQL for entities in each dataset. Missing a related table, not only creates
 * invalid data for your tests, but also is likely to cause DBUnit setUp()
 * failures from foreign key constraint violation errors. (example: If a
 * previous test had inserted INDIVIDUALS and NAME_INFO and my test tries to
 * delete only the INDIVIDUALS, the NAME_INFO.IND_ID constraint would be
 * violated)
 * <p>
 * <p>
