...
public class FlatXmlProducer extends DefaultHandler implements IDataSetProducer {
    ...
    /**
     * The DTD handler which is used to parse a DTD if available. The result of the
     * parsing is stored in {@link #_metaDataSet}.
     */
    ...
    /**
     * Whether the column sensing feature should be used to dynamically recognize
     * new columns during the parse process.
     */
     ...
     * @param dtdMetadata   Whether or not DTD metadata is available to parse via a
     *                      DTD handler
    ...
    /**
     * @param xmlSource               The input datasource
     * @param dtdMetadata             Whether or not DTD metadata is available to
     *                                parse via a DTD handler
     * @param columnSensing           Whether or not the column sensing feature
        ...
        if (!columnsToMerge.isEmpty()) {
            if (_columnSensing) {
                ...
                String msg = "Extra columns (" + extraColumnNames.toString() + ") on line " + (_lineNumber + 1)
                        + " for table " + activeMetaData.getTableName() + " (global line number is " + _lineNumberGlobal
    ...
    protected void determineAndSetRowValue(Attributes attributes, ITableMetaData activeMetaData, Object[] rowValues,
            int i) throws DataSetException, NoSuchColumnException {
