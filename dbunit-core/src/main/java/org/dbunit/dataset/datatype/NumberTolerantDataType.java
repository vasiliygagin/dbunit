...
public class NumberTolerantDataType extends NumberDataType {
    ...
    /**
     * The only method overwritten from the base implementation to compare numbers
	...
	try {
		...
		if (!this.toleratedDelta.isPercentage()) {
			...
			// TODO it would be beautiful to report a precise description about difference
			// and tolerated delta values in the assertion
		    ...
		    BigDecimal toleratedValue = bdValue1
			    .multiply(toleratedDeltaValue.divide(C_100, scale, BigDecimal.ROUND_HALF_UP));
