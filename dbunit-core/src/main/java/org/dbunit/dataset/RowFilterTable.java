...
/**
 * Filters table rows by using arbitrary column values of the table to check if
 * a row should be filtered or not. <br>
 * Implemented as a decorator for {@link ITable}.
 * 
 * See dbunit feature request at <a href=
 * "https://sourceforge.net/tracker/index.php?func=detail&aid=1959771&group_id=47439&atid=449494">#1959771</a>
 * 
 * @author gommma
...
public class RowFilterTable implements ITable, IRowValueProvider {
    ...
    /**
     * mapping of filtered rows, i.e, each entry on this list has the value of the
     * index on the original table corresponding to the desired index. For instance,
     * if the original table is: row PK Value 0 pk1 v1 1 pk2 v2 2 pk3 v3 3 pk4 v4
     * And the allowed PKs are pk2 and pk4, the new table should be: row PK Value 0
     * pk2 v2 1 pk4 v4 Consequently, the mapping will be {1, 3}
     */
    ...
    /**
     * The row that is currently checked for filtering. Used in the implementation
     * of {@link IRowValueProvider}
     */
    private int currentRowIdx;

    /**
     * Creates a new {@link ITable} where some rows can be filtered out from the
     * original table
     * 
     * @param table     The table to be wrapped
     * @param rowFilter The row filter that checks for every row whether or not it
     *                  should be filtered
    ...
    /**
     * Returns the column value for the column with the given name of the currently
     * processed row
     * 
     * @throws DataSetException
