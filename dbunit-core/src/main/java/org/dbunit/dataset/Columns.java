...
/**
 * This class exclusively provides static methods that operate on {@link Column}
 * objects.
 * 
 * @author gommma
 * @version $Revision$
...
public class Columns {
     ...
     * match one of the given <code>columnNames</code>. <br>
     * Note that this method has a bad performance compared to
     * {@link #findColumnsByName(String[], ITableMetaData)} because it iterates over
     * all columns.
     * 
     * @param columnNames the names of the columns to search.
     * @param columns     the array of columns in which the <code>columnNames</code>
     *                    will be searched.
    ...
    /**
     * Searches for the given <code>columns</code> using only the
     * {@link Column#getColumnName()} in the given <code>tableMetaData</code>
     * 
     * @param columnNames   The column names that are searched in the given table
     *                      metadata
     * @param tableMetaData The table metadata in which the columns are searched by
     *                      name
     ...
     * @throws DataSetException      if something goes wrong when trying to retrieve
     *                               the columns
     */
     ...
     * {@link Column#getColumnName()} in the given <code>tableMetaData</code>
     * 
     * @param columns       The columns whose names are searched in the given table
     *                      metadata
     * @param tableMetaData The table metadata in which the columns are searched by
     *                      name
     * @return The column objects from the given <code>tableMetaData</code>
    ...
    /**
     * Search and return the specified column from the specified column array. <br>
     * Note that this method has a bad performance compared to
     * {@link ITableMetaData#getColumnIndex(String)} because it iterates over all
     * columns.
     *
     * @param columnName the name of the column to search.
     * @param columns    the array of columns in which the <code>columnName</code>
     *                   will be searched.
     ...
     *                   only needed for the exception message in case of a
     *                   validation failure
     ...
     * 
     * @param tableName    The name of the table which is needed for the filter
     *                     invocation
     * @param columns      All available columns to which the filter will be applied
     * @param columnFilter The column filter that is applied to the given
     *                     <code>columns</code>
     ...
     * @return The columns sorted by their column names, ignoring the case of the
     *         column names
    ...
    /**
     * Merges the two arrays of columns so that all of the columns are available in
     * the result array. The first array is considered as master and if a column
     * with a specific name is available in both arrays the one from the first array
     * is used.
     * 
     * @param referenceColumns reference columns treated as master columns during
     *                         the merge
     * @param columnsToMerge   potentially new columns to be merged if they do not
     *                         yet exist in the referenceColumns
     ...
     * @return The columns that differ in the both given {@link ITableMetaData}
     *         objects
     * @throws DataSetException
     */
