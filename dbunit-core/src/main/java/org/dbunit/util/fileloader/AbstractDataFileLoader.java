...
public abstract class AbstractDataFileLoader implements DataFileLoader {
    private final Logger LOG = LoggerFactory.getLogger(AbstractDataFileLoader.class);
    ...
    public AbstractDataFileLoader(Map ro, Map rs, Map rf) {
        ...
        if (rf == null) {
            throw new IllegalArgumentException("Replacement function map is null.");
        }
    ...
    /**
     * Add the replacements in the maps (objects and substrings) to the specified
     * dataset.
     * 
     * @param ds The dataset to wrap with a <code>ReplacementDataSet</code> and
     *           process replacement tokens on.
     * @return The specified dataset decorated with <code>ReplacementDataSet</code>
     *         and processed with the tokens in the replacement maps.
     * @since 2.4.8
     */
    protected ReplacementDataSet processReplacementTokens(IDataSet ds) {
        ReplacementDataSet rds = new ReplacementDataSet(ds, replacementObjects, replacementSubstrings, functionMap);
