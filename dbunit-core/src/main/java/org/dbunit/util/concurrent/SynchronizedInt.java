...
public class SynchronizedInt extends SynchronizedVariable implements Comparable, Cloneable {
    ...
    /**
     * Make a new SynchronizedInt with the given initial value, and using its own
     * internal lock.
     **/
    ...
    /**
     * Atomically swap values with another SynchronizedInt. Uses identityHashCode to
     * avoid deadlock when two SynchronizedInts attempt to simultaneously swap with
     * each other. (Note: Ordering via identyHashCode is not strictly guaranteed by
     * the language specification to return unique, orderable values, but in
     * practice JVMs rely on them being unique.)
     * 
