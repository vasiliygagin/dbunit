 ...
 * <p>
 * Read the <a href=
 * "http://gee.cs.oswego.edu/dl/classes/EDU/oswego/cs/dl/util/concurrent/intro.html">introduction
 * to this package</a> for more details.
 * 
 * @author Doug Lea
 ...
 * @since ? (pre 2.1)
 * @see <a href=
 *      "http://gee.cs.oswego.edu/dl/classes/EDU/oswego/cs/dl/util/concurrent/Rendezvous.html">Rendezvous</a>
 * @see <a href=
 *      "http://gee.cs.oswego.edu/dl/classes/EDU/oswego/cs/dl/util/concurrent/CyclicBarrier.html">CyclicBarrier</a>
 */
public class SynchronousChannel implements BoundedChannel {
     ...
     * 
     * An arriving putter that does not already have a waiting taker creates a node
     * holding item, and then waits for a taker to take it. An arriving putter that
     * does already have a waiting taker fills the slot node created by the taker,
     * and notifies it to continue.
     * 
     * And symmetrically, two for takes:
     * 
     * An arriving taker that does not already have a waiting putter creates an
     * empty slot node, and then waits for a putter to fill it. An arriving taker
     * that does already have a waiting putter takes item from the node created by
     * the putter, and notifies it to continue.
     * 
     * This requires keeping two simple queues: waitingPuts and waitingTakes.
     * 
     * When a put or take waiting for the actions of its counterpart aborts due to
     * interruption or timeout, it marks the node it created as "CANCELLED", which
     * causes its counterpart to retry the entire put or take sequence.
     */

    /**
     * Special marker used in queue nodes to indicate that the thread waiting for a
     * change in the node has timed out or been interrupted.
     **/
