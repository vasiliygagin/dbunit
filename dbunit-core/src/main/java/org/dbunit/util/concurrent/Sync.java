...
public interface Sync {
     ...
     * Interruptions always result in `clean' failures. On failure, you can be sure
     * that it has not been acquired, and that no corresponding release should be
     * performed. Conversely, a normal return guarantees that the acquire was
     * successful.
     **/
     ...
     * <p>
     * The method has best-effort semantics: The msecs bound cannot be guaranteed to
     * be a precise upper bound on wait time in Java. Implementations generally can
     * only attempt to return as soon as possible after the specified bound. Also,
     * timers in Java do not stop during garbage collection, so timeouts can occur
     * just because a GC intervened. So, msecs arguments should be used in a
     * coarse-grained manner. Further, implementations cannot always guarantee that
     * this method will return at all without blocking indefinitely when used in
     * unintended ways. For example, deadlocks may be encountered when called in an
     * unintended context.
     * <p>
     * 
     ...
     * exceptions such as Error or NullPointerException when faced with
     * uncontinuable errors. However, these should normally only be caught by
