...
/**
 * This class implements DataType for Oracle SDO_GEOMETRY type used in Oracle
 * Spatial. See the Oracle Spatial Developer's Guide for details on
 * SDO_GEOMETRY. This class handles values similar to:
 * <ul>
 * <li>SDO_GEOMETRY(NULL, NULL, NULL, NULL, NULL)</li>
 * <li>NULL</li>
 * <li>SDO_GEOMETRY(2001, 8307, SDO_POINT_TYPE(71.2988, 42.8052, NULL), NULL,
 * NULL)</li>
 * <li>SDO_GEOMETRY(3302, NULL, SDO_POINT_TYPE(96.8233, 32.5261, NULL),
 * SDO_ELEM_INFO_ARRAY(1, 2, 1), SDO_ORDINATE_ARRAY(2, 2, 0, 2, 4, 2, 8, 4, 8,
 * 12, 4, 12, 12, 10, NULL, 8, 10, 22, 5, 14, 27))</li>
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * This class uses the following objects which were rendered using oracle jpub
 * and then slightly customized to work with dbunit:
 * <ul>
 ...
 * <li>OracleSdoElemInfoArray - corresponds to oracle SDO_ELEM_INFO_ARRAY data
 * type</li>
 * <li>OracleSdoOridinateArray - corresponds to oracle SDO_ORDINATE_ARRAY data
 * type</li>
 * </ul>
 * These classes were rendered via jpub
 * (http://download.oracle.com/otn/utilities_drivers/jdbc/10201/jpub_102.zip)
 * with the following command syntax: <code>
 ...
 * </code> The equals and hashCode methods were then added so that the objects
 * could be compared in test cases. Note that I did have to bash the jpub
 * startup script (change classpath) because it assumes oracle 10g database but
 * I ran it with 11g. Theoretically, this process can be repeated for other
 * custom oracle object data types.
 * </p>
 *
...
public class OracleSdoGeometryDataType extends AbstractDataType {
    ...
    private static final Pattern sdoGeometryPattern = Pattern
            .compile("^(?:MDSYS\\.)?SDO_GEOMETRY\\s*\\(\\s*([^,\\s]+)\\s*,\\s*([^,\\s]+)\\s*,\\s*");
    private static final Pattern sdoPointTypePattern = Pattern.compile(
            "^(?:(?:(?:MDSYS\\.)?SDO_POINT_TYPE\\s*\\(\\s*([^,\\s]+)\\s*,\\s*([^,\\s]+)\\s*,\\s*([^,\\s\\)]+)\\s*\\))|(NULL))\\s*,\\s*");
    private static final Pattern sdoElemInfoArrayPattern = Pattern
            .compile("^(?:(?:(?:(?:MDSYS\\.)?SDO_ELEM_INFO_ARRAY\\s*\\(([^\\)]*)\\))|(NULL)))\\s*,\\s*");
    private static final Pattern sdoOrdinateArrayPattern = Pattern
            .compile("^(?:(?:(?:(?:MDSYS\\.)?SDO_ORDINATE_ARRAY\\s*\\(([^\\)]*)\\))|(NULL)))\\s*\\)\\s*");
    ...
    public Object typeCast(Object value) throws TypeCastException {
        ...
        if (value instanceof String) {
            ...
            try {
                ...
                BigDecimal gtype = NULL.equals(sdoGeometryMatcher.group(1)) ? null
                        : new BigDecimal(sdoGeometryMatcher.group(1));
                BigDecimal srid = NULL.equals(sdoGeometryMatcher.group(2)) ? null
                        : new BigDecimal(sdoGeometryMatcher.group(2));
                ...
                if (NULL.equals(sdoElemInfoArrayMatcher.group(2))) {
                    ...
                    if (elemInfoStrings.length == 1 && "".equals(elemInfoStrings[0])) {
                        ...
                        for (int index = 0; index < elemInfoStrings.length; index++) {
                            elemInfos[index] = NULL.equals(elemInfoStrings[index]) ? null
                                    : new BigDecimal(elemInfoStrings[index]);
                        }
                ...
                if (NULL.equals(sdoOrdinateArrayMatcher.group(2))) {
                    ...
                    if (ordinateStrings.length == 1 && "".equals(ordinateStrings[0])) {
                        ...
                        for (int index = 0; index < ordinateStrings.length; index++) {
                            ordinates[index] = NULL.equals(ordinateStrings[index]) ? null
                                    : new BigDecimal(ordinateStrings[index]);
                        }
                        sdoOrdinateArray = new OracleSdoOrdinateArray(ordinates);
                    }
                }

                OracleSdoGeometry sdoGeometry = new OracleSdoGeometry(gtype, srid, sdoPoint, sdoElemInfoArray,
                        sdoOrdinateArray);
    ...
    public void setSqlValue(Object value, int column, PreparedStatement statement)
        ...
        if (castValue == null) {
            statement.setNull(column, OracleSdoGeometry._SQL_TYPECODE, OracleSdoGeometry._SQL_NAME);
        } else {
     ...
     * after the typeCast because OracleSdoGeometry objects are not Comparables but
     * can test for equality (via equals method.) It is needed for test cases that
     * check for equality between data in xml files and data read from the database.
     */
