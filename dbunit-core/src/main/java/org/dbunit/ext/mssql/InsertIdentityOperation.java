...
/**
 * This class disable the MS SQL Server automatic identifier generation for the
...
public class InsertIdentityOperation extends AbstractOperation {
    ...
    public static final DatabaseOperation CLEAN_INSERT = new CompositeOperation(DatabaseOperation.DELETE_ALL,
    ...
    private static final IColumnFilter DEFAULT_IDENTITY_FILTER = (tableName, column) -> column.getSqlTypeName()
            .endsWith("identity");
     ...
     * <li>GENERATED ALWAYS AS IDENTITY</li>
     * </ul>
     * set which is the SQL standard syntax to describe auto-generated key columns.
     * Also accepts columns that have the auto-increment property set to
     * <code>true</code> (note that it does not yet have the ability to check
     * whether the column is a primary key col).
     *
     * @since 2.4.3
     */
    public static final IColumnFilter IDENTITY_FILTER_EXTENDED = (tableName, column) -> {
        String remarks = column.getRemarks();
        boolean isIdentityCol = (remarks != null) && (remarks.indexOf("GENERATED BY DEFAULT AS IDENTITY") > -1
                || remarks.indexOf("GENERATED ALWAYS AS IDENTITY") > -1);

        // If "remarks" did not give the appropriate hint, check the auto-increment
        // property
        if (!isIdentityCol) {
    ...
    /**
     * Creates a new InsertIdentityOperation object that decorates the specified
     * operation.
     */
    ...
    boolean hasIdentityColumn(ITableMetaData metaData, IDatabaseConnection connection) throws DataSetException {
        ...
        IColumnFilter identityFilter = connection.getDatabaseConfig().getIdentityFilter();
        ...
        for (Column column : columns) {
            if (identityFilter.accept(metaData.getTableName(), column)) {
                return true;
            }
        }
